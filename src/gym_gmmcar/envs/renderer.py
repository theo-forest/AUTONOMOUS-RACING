...
class _Renderer(object):
    ...
    def __init__(self, params, env_type, radius=1.0):
        """
        ...
        # Dynamic limits based on radius
        limit = self.radius * 1.5
        ...
        if env_type == 'StraightEnv':
            ...
            # arc diameter is 2 * radius
            arc = patches.Arc((0, 0), 2*self.radius, 2*self.radius, angle=360, color='c', ls=':')
            self._ax.add_patch(arc)
        elif env_type == 'FastCircleEnv' or env_type == 'OttEnv' or env_type == 'CircleEnv':
            eps = 0.5 # Increased visualization epsilon for visibility at large scales
            r = self.radius
            # Outer boundary
            arc = patches.Arc((0, 0), 2*(r+eps), 2*(r+eps), angle=360, color='r', ls=':')
            self._ax.add_patch(arc)
            # Ideal path
            arc = patches.Arc((0, 0), 2*r, 2*r, angle=360, color='c')
            self._ax.add_patch(arc)
            # Inner boundary
            arc = patches.Arc((0, 0), 2*(r-eps), 2*(r-eps), angle=360, color='r', ls=':')
            self._ax.add_patch(arc)
